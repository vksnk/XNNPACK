#!/usr/bin/env python3
"""Autotuner calibration campaign on FP32SoftmaxDecomp masks 1..7.

Per mask: record space -> sweep -> random -> hillclimb via autotune.py,
stock scheduler (YNN_UNFUSE=0), threads=4, one lane per process group.
"""
...
BENCH = f"{ROOT}/bazel-bin/bench/subgraph/softmax"
...
def bench_cmd(mask, min_time="0.3s"):
    return [BENCH,
            f"--benchmark_filter=FP32SoftmaxDecomp/M:128/N:256/K:512/NormMask:{mask}/",
