...
// Reuses allocations across pipeline evaluations. Slinky allocates and frees
// every internal buffer on every evaluation; for the large intermediates of
// a prefill-sized pipeline that means tens of MB of malloc/munmap per
// invocation, which both stalls the evaluating thread (all other threads
// have nothing to do while it happens) and repays the page-fault cost of
// first-touch on every evaluation. Pipelines run one at a time, so a single
// process-wide pool bounds retained memory by the peak footprint of one
// evaluation rather than the sum over all runtimes.
...
bool allocation_pool_enabled() {
  static const bool enabled = []() {
    const char* env = getenv("YNN_ALLOC_CACHE");
...
ynn_runtime::ynn_runtime(ynn::ref_count<const ynn_subgraph> subgraph,
  ...
  if (allocation_pool_enabled()) {
    ...
    eval_config.free = [](slinky::var sym, slinky::raw_buffer* buffer,
                          void* ptr) {
      allocation_pool::global().release(ptr);
    };
  } else {
